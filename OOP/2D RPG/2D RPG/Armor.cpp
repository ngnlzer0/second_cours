...
Armor::Armor(int pos_x, int pos_y, double g_weight, double g_block, double g_missing, int g_durability, Armor_type g_type, bool random)
	: BaseItem("", 5.0), type(g_type), weight(Set_weight(g_weight)), block(Set_block(g_block)), miss(Set_missing(g_missing)), durability(Set_durability(g_durability))
{
	...
	setNewTexture(armor_texture_path);
	set_Weight_i(weight);
